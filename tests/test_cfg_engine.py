...
def test_fetch_move_y(basic_warehouse):
    ...
    action = get_next_action_from_egglog(
        basic_warehouse, cy=0.0, cx=0.0, holding=False, 
        phase="fetch", cmd_type="FETCH", target_id=1
    )
...
def test_fetch_move_x(basic_warehouse):
    ...
    action = get_next_action_from_egglog(
        basic_warehouse, cy=20.0, cx=0.0, holding=False, 
        phase="fetch", cmd_type="FETCH", target_id=1
    )
...
def test_fetch_pick(basic_warehouse):
    """Test pick action."""
    action = get_next_action_from_egglog(
        basic_warehouse, cy=20.0, cx=10.0, holding=False, 
        phase="fetch", cmd_type="FETCH", target_id=1
    )
...
def test_search_lock(basic_warehouse):
    ...
    action = get_next_action_from_egglog(
        basic_warehouse, cy=0.0, cx=0.0, holding=True, 
        phase="deliver", cmd_type="SEARCH_TARGET", target_type="queue"
    )
...
def test_deliver_move_y(basic_warehouse):
    ...
    action = get_next_action_from_egglog(
        basic_warehouse, cy=0.0, cx=0.0, holding=True, 
        phase="deliver", cmd_type="DELIVER", target_type="queue", locked_id="Q1"
    )
    assert action["type"] == "update_y"
